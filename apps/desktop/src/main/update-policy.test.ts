...
describe("update policy", () => {
  ...
  it("disables updates outside a packaged application", () => {
    expect(
      initialUpdateStatus(false, "0.15.0-alpha.1", "2026-08-06T00:00:00Z"),
    ).toEqual({
      state: "DISABLED",
      current_version: "0.15.0-alpha.1",
      message: "Updates are disabled for development builds.",
      checked_at: "2026-08-06T00:00:00Z",
    });
    expect(initialUpdateStatus(true, "0.15.0-alpha.1").state).toBe("IDLE");
  });
});
