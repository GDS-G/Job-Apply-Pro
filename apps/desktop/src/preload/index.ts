...
contextBridge.exposeInMainWorld("jobApplyPro", {
  ...
  workbench: {
    ...
    executeGreenhouseFormAction: (input: GreenhouseFormActionInput) =>
      ipcRenderer.invoke("greenhouse-form-action:execute", input),
    ...
    reconcileBrowserNavigation: (operationId: string, sessionId: string) =>
      ipcRenderer.invoke(
        "workbench:reconcile-browser-navigation",
        operationId,
        sessionId,
      ),
    reconcileBrowserUpload: (operationId: string, sessionId: string) =>
      ipcRenderer.invoke(
    ...
    reviewLinkedInJobIdentity: (runId: string) =>
      ipcRenderer.invoke("portals:review-linkedin-job-identity", runId),
