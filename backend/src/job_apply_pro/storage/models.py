...
class DocumentGenerationAuditRow(Base):
    ...
    template: Mapped[str] = mapped_column(String(30), default="PROFESSIONAL")
    ranking_mode: Mapped[str] = mapped_column(String(30), default="DETERMINISTIC")
    ranking_method: Mapped[str] = mapped_column(String(80), default="DETERMINISTIC_TOKEN_OVERLAP")
...
class ApplicationAnswerRow(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True)
    application_id: Mapped[str] = mapped_column(ForeignKey("applications.id"), index=True)
    profile_id: Mapped[str | None] = mapped_column(ForeignKey("candidate_profiles.id"), index=True)
    ...
    source_type: Mapped[str] = mapped_column(String(40), default="LEGACY")
    source_answer_id: Mapped[str | None] = mapped_column(String(36))
    library_answer_id: Mapped[str | None] = mapped_column(ForeignKey("answer_library.id"))
    evidence_claim_ids_json: Mapped[list[str]] = mapped_column(JSON, default=list)
    retrieval_results_json: Mapped[list[dict[str, object]]] = mapped_column(JSON, default=list)
    provider_id: Mapped[str | None] = mapped_column(String(80))
    model_id: Mapped[str | None] = mapped_column(String(120))
    prompt_version: Mapped[str | None] = mapped_column(String(40))
    policy_version: Mapped[str] = mapped_column(String(40), default="answer-drafting/1.0")
    confidence: Mapped[float] = mapped_column(Float)
    encrypted_generated_value: Mapped[str | None] = mapped_column(Text)
    character_limit: Mapped[int] = mapped_column(Integer, default=20_000)
    character_limit_applied: Mapped[bool] = mapped_column(Boolean, default=False)
    ...
    reuse_permission: Mapped[str] = mapped_column(String(30), default="APPLICATIONS")
