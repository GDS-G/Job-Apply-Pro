...
## Reviewed Browser Field Reconciliation build

The active source milestone is **Reviewed Browser Field Reconciliation `v0.71.0-alpha.1`**. Before an eligible reviewed field write crosses the Playwright boundary, the runtime now stores an encrypted intent tied to the exact durable effect operation, attempt, request fingerprint, source page fingerprint, action kind, and deterministic verification. If the worker response is lost and the effect becomes `UNCERTAIN`, Operations may offer **Verify current field outcome** only while the same visible page remains in user takeover. Preview and approval each perform a read-only postcondition check; approval records local evidence without typing, selecting, checking, clicking, navigating, uploading, submitting, or retrying. The native warning defaults to Cancel. The original uncertain result remains immutable history, the browser stays in takeover, and only the resolved-attention projection changes. Older effects without an encrypted intent and all non-field actions remain blocked. See [ADR-0081](docs/adr/0081-reviewed-browser-field-reconciliation.md), [ADR-0080](docs/adr/0080-reviewed-greenhouse-single-select-widgets.md), and [ADR-0073](docs/adr/0073-durable-external-effect-ledger.md).
...
Offline restore retains exclusive workspace ownership, encrypted intent and persistent startup admission before replacement. V2 records include every target's before/after image and durable terminal evidence, so an interrupted operation can explicitly converge to its exact original bytes without guessing or merging history. The original key and recovery records remain required. Legacy v1 evidence is completion/manual-recovery only. This is still not atomic multi-file replacement, automatic rollback, forward-apply resume or a physical power-loss guarantee; see [ADR-0066](docs/adr/0066-durable-restore-admission.md) and [ADR-0071](docs/adr/0071-durable-restore-rollback.md).
