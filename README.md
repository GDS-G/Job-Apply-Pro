...
## Durable Restore Rollback build

The active source milestone is **Durable Restore Rollback `v0.61.0-alpha.1`**. Every v2 offline restore seals authenticated before/after images for the database and document targets before activating the startup guard. Preparation authenticates the complete staged source before querying SQLite, preserves the compiled schema-0027 recorded-history/dependency graph from one exact original byte snapshot, rechecks the final serialized database and then the live sealed targets, confines bookkeeping to a default-deny authorizer-restricted in-memory SQLite image, captures plaintext files sequentially, and applies bounded per-record/operation retained-evidence admission plus exact pre-guard cleanup. An interrupted operation can be inspected and rolled back only through an explicit reviewed fingerprint; unknown live bytes, missing evidence, a completed apply receipt or a cleared guard fail closed. See [ADR-0071](docs/adr/0071-durable-restore-rollback.md).

When a packaged start finds an active guard, Electron uses the existing `safeStorage`-protected key in an isolated native recovery-only session. Normal startup, migrations, API services, renderer creation and updates stay off. **Keep workspace blocked** is the default; the user must explicitly select exact rollback or authenticated terminal-state finalization, after which the controller reinspects the result and guard. The v0.61 source suite, backend-dist and installer-payload smokes, delivered supervisor, and source-controller-to-delivered-backend recovery protocol are validated. Protected CI/security, a physical installed-app recovery rehearsal with real Windows key storage/dialogs, signing and live acceptance remain. The milestone stays alpha and is not a signed release claim.
