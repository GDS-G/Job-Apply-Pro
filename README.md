...
## Durable Media Recovery build

The active source milestone is **Durable Media Recovery `v0.51.0-alpha.1`**. Gemini media requests now commit per-file cleanup intent before external upload and encrypt known remote resource names. A durable credential-identity gate prevents another media invocation under the same provider kind, trusted origin, and exact API key while cleanup is unresolved, even if the provider configuration is renamed or aliased. Startup and periodic recovery attempt deletion only for known resources with that matching credential identity; unknown upload outcomes require verified manual review. Different API keys cannot establish that two configurations belong to the same remote account, and rotation leaves earlier obligations retained. The **Durable media recovery** panel exposes safe status and recovery actions without media bytes, resource names, URLs, API keys, or filenames. See ADR-0061 and the [user guide](docs/user-guide.md#review-durable-media-recovery).

The existing explicit-consent, 5 MiB, four-image, multimodal-capability, stateless-interaction, and terminal retention-error boundaries remain in force. Leases are renewed between lifecycle phases; they are not hard real-time deadlines. Deletion is attempted, never guaranteed after network or credential failure. Local validation passes 320 backend tests at 84.45% coverage and 79 desktop tests. The unsigned `Job-Apply-Pro-0.51.0-alpha.1-x64.exe` builds and its bundled backend passes migration, cleanup API, encrypted backup and offline restore smoke. See the readiness audit for hashes and PR #82 for protected checks; this is not signed or live-provider acceptance.
