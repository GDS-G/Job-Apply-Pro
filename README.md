...
## Actionable Desktop Notifications build

The active milestone is **Actionable Desktop Notifications `v0.19.0-alpha.1`**. It adds a privacy-safe in-app notification center and an opt-in Windows notification channel for MFA, CAPTCHA, user intervention, approaching assessment deadlines, workflow/session failures or expiry, recruiter/interview/offer messages, due follow-ups, backup failures, and update failures. Electron main polls existing authenticated local contracts every 60 seconds, limits active and per-refresh delivery counts, coalesces concurrent refreshes, persists bounded deduplication identifiers, and routes clicks only to fixed workbench destinations. OS notifications contain generic action text rather than employer, candidate, sender, subject, message, or diagnostic content. It retains encrypted provider configuration, bounded synchronization and document ingestion, evidence-bound generation, supervised portal execution, and the Portal Readiness baselines.
