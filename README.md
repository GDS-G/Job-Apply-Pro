...
## Reviewed Greenhouse Single-Select Widgets build

The active source milestone is **Reviewed Greenhouse Single-Select Widgets `v0.70.0-alpha.1`**. A current Greenhouse form assessment may now classify one narrowly bounded accessible single-select widget as `REVIEW_FIELD`: a visible enabled input combobox that is already expanded, controls exactly one visible non-multiselect listbox, has a unique exact semantic locator, and exposes non-empty case-insensitively unique visible enabled option labels. The dedicated `SINGLE_SELECT_WIDGET` binding kind accepts only reviewed yes/no or multiple-choice answers. Execution rechecks the current page and form reviews, resolves the option only inside the combobox's one current controlled listbox, verifies the value, redacts the sensitive answer from persistence, and never reopens a widget that changed to collapsed. Other custom, multiselect, legal, signature, repeated, hidden, ambiguous and cross-portal widgets remain direct user actions. See [ADR-0080](docs/adr/0080-reviewed-greenhouse-single-select-widgets.md), [ADR-0079](docs/adr/0079-reviewed-greenhouse-final-submission.md), and [ADR-0078](docs/adr/0078-reviewed-greenhouse-native-fields.md).

The guide suggests navigation through readiness, evidence, application fields, challenges, and follow-up; it does not advance state or grant permission. Backend `allowed_controls`, exact review fingerprints, provider configuration, durable effect admission, and service policies remain authoritative. An unresolved browser effect links to Operations without exposing a generic retry, clear, or resolve mutation. Production portal submission remains disabled.

The v0.70 sanitized and loopback tests cover bounded/unsafe widget classification, dedicated binding compatibility, provider-scoped required-field coverage, exact reviewed option execution, cross-portal/collapsed/unowned/multiselect refusal, disabled-option exclusion, renderer mapping, and live worker refusal after the widget changes to collapsed. Exact source checkpoint `c4db1aa232ca6e2d8bb2815faf33702b0c74aebb` passes **1,863 backend tests** at **86.82% coverage** across **16,850 statements** and **492 default desktop tests** with two explicit packaged-runtime opt-ins skipped. Ruff lint/format passes **239 files**; strict mypy passes **208 source/test/helper files**. Workspace Prettier, oxlint, TypeScript, shared-contract type/lint/build/test commands and the production Electron build pass under Node 24.19.0, pnpm 11.19.0 and Python 3.12.14. The contracts test command currently has no runtime test files and exits successfully by declaration. The changed-content scan finds zero common secret assignments and zero private-key markers. Greenhouse remains `production_enabled=false`, its live-validated page-type list remains empty and supervised/final-submit execution remain disabled by default. Exact v0.70 package/protocol, protected, signed, authorized live-provider and physical installed-app evidence remain pending; no older result transfers.
