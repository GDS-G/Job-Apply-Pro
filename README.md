...
## Durable External-Effect Ledger build

The active source milestone is **Durable External-Effect Ledger `v0.63.0-alpha.1`**. Browser actions and AI completion/embedding provider calls now commit privacy-safe logical ownership and a `DISPATCHING` attempt before external I/O. Exact replays are consumed, changed claims conflict, and ambiguous responses, lost local evidence or interrupted dispatches become `UNCERTAIN` without automatic retry or provider fallback. Browser uncertainty forces user takeover. AI cache hits remain outside provider-attempt accounting. See [ADR-0073](docs/adr/0073-durable-external-effect-ledger.md).

The ledger stores only bounded identifiers, keyed fingerprints, safe codes, counts and evidence references—never prompts, candidate data, page content, media, credentials, tokens or raw provider errors. Schema `20260913_0029` adds the two ledger tables and advances forward-restore inspection to 46 protected tables with exact replay-authority and semantic consistency checks. Calendar UPDATE remains reserved but disabled. The specialized mail, calendar CREATE and Gemini-cleanup journals remain authoritative for their existing effect families.

The exact v0.63 source checkpoint passes 1,809 backend tests at 86.69% coverage and 479 default desktop tests with two explicit packaged-runtime opt-ins skipped, plus Ruff, strict mypy, Prettier, oxlint, TypeScript and the production Electron build. Operations now shows bounded, privacy-safe unresolved effect records and intentionally provides no generic retry or clear mutation. The v0.63 unsigned package candidate, protected CI/security, authorized live-provider use and physical installed-app acceptance remain separate. The exact v0.62 package evidence remains recorded historically in the production-readiness audit and does not transfer to v0.63.
