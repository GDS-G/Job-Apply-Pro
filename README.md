...
## Reviewed Greenhouse Form Actions build

The active source milestone is **Reviewed Greenhouse Form Actions `v0.67.0-alpha.1`**. A ready saved Greenhouse application can now preview and natively confirm either the exact selected-resume upload or one uniquely reviewed later-stage navigation. The backend recomputes workflow/readiness/page authority, derives the URL, locator, immutable document version, filename and SHA-256, verifies document bytes before plaintext staging, and records the action through the durable external-effect ledger. The renderer supplies none of those privileged values. See [ADR-0077](docs/adr/0077-reviewed-greenhouse-form-actions.md), [ADR-0076](docs/adr/0076-greenhouse-form-execution-contracts.md), and [ADR-0075](docs/adr/0075-reviewed-greenhouse-application-launch.md).
...
The v0.67 sanitized tests cover upload and navigation approval/cancellation, stale previews, exact document-byte binding, changed controls/pages/origins, postcondition failure, user takeover, no automatic retry, and final-confirmation separation. Exact source checkpoint `794e5b8face437460e65ef05377fd8ab6422415c` passes 1,832 backend tests at 86.88% coverage and 490 desktop tests with two packaged-runtime opt-ins skipped; full static analysis, shared-contract checks and the production Electron build pass. This source slice does not establish live compatibility. Package, protected CI/security, signing, authorized live-provider/portal use, and physical installed-app acceptance remain separate; no older package or protected result transfers.
