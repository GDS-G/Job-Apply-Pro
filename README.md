...
When a packaged start finds an active guard, Electron uses the existing `safeStorage`-protected key in an isolated native recovery-only session. Normal startup, migrations, API services, renderer creation and updates stay off. **Keep workspace blocked** is the default; the user must explicitly select exact rollback or authenticated terminal-state finalization, after which the controller reinspects the result and guard. The v0.61 source suite is validated; exact packaged-backend, delivered-package, protected-CI and physical Windows validation remain. The milestone stays alpha and is not a signed release claim.
