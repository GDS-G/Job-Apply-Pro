...
## Greenhouse Application Vertical Slice build

The active source milestone is **Greenhouse Application Vertical Slice `v0.65.0-alpha.1`**. A saved public Greenhouse job can now move from exact-source readiness and immutable resume selection into a visible supervised browser only through a backend-derived, fingerprint-bound, cancel-default reviewed launch. The renderer cannot supply the workflow, start URL, or origins, generic Greenhouse start is rejected, and the first durable portal evidence step binds the launch review to the observed page. See [ADR-0075](docs/adr/0075-reviewed-greenhouse-application-launch.md).
...
The exact local v0.65 source worktree passes 1,820 backend tests at 86.79% coverage and 485 default desktop tests with two explicit packaged-runtime opt-ins skipped, plus Ruff across 236 files, strict mypy across 205 source/test/helper files, Prettier, oxlint, TypeScript, contracts build, and the production Electron build. This is source evidence only. The v0.65 package, protected CI/security, signing, authorized live-provider/portal use, and physical installed-app acceptance remain separate; no older package or protected result transfers to this commit.
