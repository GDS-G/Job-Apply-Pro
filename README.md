...
## Reviewed Greenhouse Final Submission build

The active source milestone is **Reviewed Greenhouse Final Submission `v0.69.0-alpha.1`**. The separate default-off final-submit gate now composes with a current Greenhouse `FINAL_SUBMISSION_GATE` contract. The backend freshly requires the exact review stage, form fingerprint, one unique safe semantic locator, both global policy gates, and one elevated confirmed action. A click never establishes success: the existing catalog must recognize a Greenhouse confirmation page and extract an identifier, otherwise the result is uncertain and returns to user takeover without automatic retry. The renderer supplies no URL, locator, control, action, phrase, or confirmation evidence. See [ADR-0079](docs/adr/0079-reviewed-greenhouse-final-submission.md), [ADR-0078](docs/adr/0078-reviewed-greenhouse-native-fields.md), and [ADR-0077](docs/adr/0077-reviewed-greenhouse-form-actions.md).
...
The v0.69 sanitized tests cover missing/stale Greenhouse form review, multiple and unsafe submit controls, cross-portal review injection, unrecognized request authority, exact-locator dispatch, renderer composition, off-origin and identifier-free uncertainty, no retry after an ambiguous click, and identifier-backed confirmation. Exact source checkpoint `10c981d27f49a1acefa9401f6e020b67eba07afe` passes **1,846 backend tests** at **86.93% coverage** and **491 default desktop tests** with two explicit packaged-runtime opt-ins skipped. Ruff lint/format passes **239 files**; strict mypy passes **208 source/test/helper files**. Workspace Prettier, oxlint, TypeScript, shared-contract type/lint/build/test commands and the production Electron build pass; the contracts test command currently has no runtime test files and exits successfully by declaration. The unsigned v0.69 backend distribution, unpacked app, NSIS installer, delivered-backend smoke, supervisor lifecycle, and recovery-controller protocol also pass local qualification under Node 24.19.0 and Python 3.12.14. This evidence does not establish live compatibility. Protected CI/security, Authenticode signing, authorized live-provider/portal use, and physical installed-app acceptance remain separate; no older package or protected result transfers.
