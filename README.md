...
## Guided Application Workspace build

The active source milestone is **Guided Application Workspace `v0.64.0-alpha.1`**. The durable queue is now the desktop's single application-selection authority: candidate evidence, application answers, field bindings/executions, document previews, Reference ATS and supervised portal runs, challenges, correlated communications, and selected-browser uncertainty are scoped to the same saved workflow. Late responses from a previous selection are ignored, and every application selector follows the queue selection. See [ADR-0074](docs/adr/0074-guided-application-workspace.md).
...
The exact local v0.64 source worktree passes 1,811 backend tests at 86.77% coverage and 480 default desktop tests with two explicit packaged-runtime opt-ins skipped, plus Ruff across 233 files, strict mypy across 202 source/test/helper files, Prettier, oxlint, TypeScript, contracts build, and the production Electron build. This is source evidence only. The v0.64 package, protected CI/security, signing, authorized live-provider/portal use, and physical installed-app acceptance remain separate; no older package or protected result transfers to this commit.
