...
## Packaged Browser Runtime build

The active source milestone is **Packaged Browser Runtime `v0.53.0-alpha.1`**. The Windows package now contains a dedicated hidden browser worker with real JSON-lines pipes, fixing the frozen Python-launch mismatch. Worker and packaged API behavior are verified against isolated loopback fixtures; see [ADR-0063](docs/adr/0063-packaged-browser-runtime.md). This does not establish live portal compatibility or complete desktop lifecycle recovery. Uploaded JPEG, PNG, and WebP images are fully decoded locally, checked against dimensions/frame limits, oriented, and re-encoded as metadata-free PNG before gateway hashing or provider transfer. Both original and normalized bytes must fit 5 MiB; requests allow at most four images. Animation, corrupt/incomplete images, edges above 8,192 pixels, and images above 16,777,216 pixels are rejected. No resizing or visual-content redaction is implied. See [ADR-0062](docs/adr/0062-validated-media-inputs.md).
