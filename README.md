...
## Reviewed Greenhouse Native Fields build

The active source milestone is **Reviewed Greenhouse Native Fields `v0.68.0-alpha.1`**. A current Greenhouse `REVIEW_FIELD` contract can now compose with the existing reviewed answer/binding workflow for exactly one native field per cancel-default confirmation. The backend freshly recomputes page, form, control, locator, answer revision and binding authority before decrypting the answer or dispatching the sensitive-value action. The renderer supplies no URL, locator, answer value, control metadata, action class, or confirmation phrase. Custom/legal controls receive an explicit visible-browser handoff and require a fresh capture; they are never inferred or marked complete. See [ADR-0078](docs/adr/0078-reviewed-greenhouse-native-fields.md), [ADR-0077](docs/adr/0077-reviewed-greenhouse-form-actions.md), and [ADR-0076](docs/adr/0076-greenhouse-form-execution-contracts.md).
...
The v0.68 sanitized tests cover missing/stale Greenhouse form review, already-satisfied or foreign controls, cross-portal review injection, unrecognized request authority, sensitive-value handling, renderer composition and explicit manual handoff. Exact source checkpoint `3482871d0a7f910a7851fff3440372f8f2403175` passes 1,835 backend tests at 86.88% coverage and 490 desktop tests with two packaged-runtime opt-ins skipped; Ruff lint/format passes 239 files, strict mypy passes 208 files, and workspace formatting, oxlint, TypeScript, shared-contract commands and the production Electron build pass. This source slice does not establish live compatibility. Package, protected CI/security, signing, authorized live-provider/portal use, and physical installed-app acceptance remain separate; no older package or protected result transfers.
