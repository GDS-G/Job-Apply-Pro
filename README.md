...
## Document Generation & Retention build

The active milestone is **Document Generation & Retention `v0.15.0-alpha.1`**. It adds fingerprint-reviewed DOCX/PDF resume and cover-letter generation from locked application-approved evidence, encrypted generated versions, requirement-gap visibility, and exact submitted-document retention. It retains the supervised portal, encrypted OAuth/PKCE provider connectivity, and Portal Readiness baselines.
...
The Candidate Knowledge API is under `/api/v1/knowledge`. Supported imports are PDF, DOCX, RTF, TXT, and Markdown. Tailored DOCX/PDF generation requires a selected application and uses only locked verified claims approved for applications; the user reads every output paragraph and requirement gap before a fingerprint-bound native confirmation. Generated bytes and extraction data are encrypted, while an optional native save dialog exports the chosen result through Electron main without exposing bytes to the renderer. Confirmed portal uploads retain the selected version, displayed filename, SHA-256 digest, role, and upload fingerprint. Legacy binary `.doc` files must still be converted with a trusted local office tool before import.
