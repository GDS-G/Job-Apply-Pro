...
## Verified Mail Attachments build

The active source milestone is **Verified Mail Attachments `v0.55.0-alpha.1`**. Reviewed Gmail/Outlook drafts bind the current account connection and exact encrypted PDF/DOCX versions. Files are resolved and checked again before sending; durable draft claims prevent a second local send attempt. Successful provider responses mean **accepted**, not delivered. Outlook uses the scope-compatible `Mail.Send` endpoint. The desktop provides native review of the exact recipient, content and attachment manifest. See [ADR-0065](docs/adr/0065-verified-mail-attachments.md) for bounds, recovery safeguards and limitations; the readiness audit records exact-version validation.
...
Backup archives are encrypted before storage, authenticated with the local master key, and verified by archive and per-entry SHA-256 hashes. Database replacement is excluded from the live API: the app stages and fingerprints restore plans, then its privileged supervisor stops the backend before invoking the offline recovery command. Replacement is blocked when the original current database has unresolved provider-media cleanup, or cannot be safely inspected; restoring an older backup cannot discard those obligations. The prior database is retained as a `.pre-restore` file. Production release jobs fail closed without a Windows signing certificate.
