...
## Calendar Attempt Admission build

The active source milestone is **Calendar Attempt Admission `v0.62.0-alpha.1`**. A calendar CREATE plan is bound to the exact provider account, OAuth connection epoch, primary-calendar target, reviewed event, fixed private/busy/no-reminders/no-invitations policy, policy version and provider wire contract. The app rejects attendees/invitations, conferencing creation, recurrence, caller-selected provider IDs and UPDATE before claiming or contacting a provider. One permanent `PLANNED` claim is committed before token resolution or transport; exact retries replay it, while a different key or an ambiguous recovered outcome cannot dispatch again. Google and Microsoft receive provider-native duplicate-defense keys derived from the claimed attempt. See [ADR-0072](docs/adr/0072-calendar-attempt-admission.md).

Execution remains a user-reviewed desktop action. The native dialog defaults to cancel and must show the provider, account, primary calendar, title, time range, zone, location and fixed private/busy/no-reminders/no-invitations policy from a freshly fetched plan. Provider acceptance plus a validated returned identifier means `CONFIRMED`, not invitation delivery or later readback. Typed definitely-not-applied refusals become `FAILED`; transport, timeout, malformed-response, unexpected and lost-terminal-write outcomes remain consumed `UNCERTAIN` or `PLANNED` attempts for reconciliation. Calendar UPDATE, unattended scheduling, authorized live-provider acceptance, signing and physical Windows validation remain separate work.

The exact local v0.62 gate passes 1,774 backend tests, 479 default desktop tests, formatting/lint/type checks, the production build, both dependency audits, both frozen-backend package copies and the delivered supervisor/recovery protocols. The artifact hashes and unsigned status are recorded in the production-readiness audit; protected integration, authorized live-provider use and physical installed-app acceptance remain separate.
