...
The first compiled probe exposed a second frozen-runtime mismatch: Playwright defaulted its browser cache to bundle-local `.local-browsers`, while controlled Chromium was installed in the normal per-user cache. `_configure_worker_browser_cache()` now runs before driver startup. Only frozen Windows operation with no explicit `PLAYWRIGHT_BROWSERS_PATH` selects the absolute `%LOCALAPPDATA%/ms-playwright` cache; an invalid or absent local-app-data root fails safely. Explicit overrides (including `0` for bundled browsers), source operation and non-Windows behavior remain unchanged. This does not install/download a browser. Installed Edge remains the normal desktop channel; controlled package tests exercise Chromium.
