...
Automated coverage includes staged-byte hashing and cleanup, pre-dispatch encrypted intent, persisted-path redaction, response-loss success, same-fingerprint/absent/duplicate refusal, approval-time reproof, immutable uncertain history, authenticated API identity, client routing, strict IPC UUIDs, Cancel-default native review, renderer kind selection, and kind-specific restore authentication. Exact source checkpoint `22fe11e7276089a46b65f3ec0a41d566a45ebfc5` passes 1,926 backend tests at 86.59% coverage across 17,853 statements and 521 default desktop tests with two explicit package-only opt-ins skipped. Static, production-build and dependency-audit gates pass. Candidate checkpoint `909c3fe6d579881bedc9ffb2c5e60c22c5840646` passes both packaged-backend smokes and both installer-delivered protocols; artifact hashes are in the readiness audit. Every executable is unsigned, so this is not production acceptance.
