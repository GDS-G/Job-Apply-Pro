...
| Gmail, Outlook, Google Calendar, and Outlook Calendar | Implemented, externally gated | Common contracts; validated/encrypted registration import and sanitized status; environment precedence; encrypted OAuth/PKCE session and token persistence; refresh/revocation; official adapters; bounded pagination and response limits; encrypted account-bound Gmail history and Outlook Inbox delta state; reset recovery; authenticated mail sync; encrypted provider/message deduplication; Outlook attachment-name metadata without content download; consent/scope/sync UI; API, migration, storage, desktop, and HTTP provider tests | Owner/provider client registration, Google verification or Microsoft administrator consent where required, sandbox/live validation, attachment-content policy, webhook/push infrastructure and policy if pursued, quota/rate-limit approval, and live evidence |
| Application tracking, reports, model cost, portal health, diagnostics, help, daily scheduling, and desktop notifications | Implemented and automated for local records | Operations/communication/support services, renderer dashboard, privacy-safe notification collector/manager, assessment deadlines, bounded persistent deduplication, fixed-destination activation, export/reconciliation/notification tests | Confirmed upcoming-interview reminders from calendar state, broader UX depth, real-provider health, physical Windows notification/sleep/Focus Assist evidence, and user acceptance testing |
