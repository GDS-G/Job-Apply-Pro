...
| Gmail, Outlook, Google Calendar, and Outlook Calendar | Implemented, externally gated | Common contracts; encrypted OAuth/PKCE session and token persistence; refresh/revocation; official Gmail, Microsoft Graph, Google Calendar, and Outlook Calendar adapters; consent/scope UI; HTTP provider tests | Owner/provider client registration, Google verification or Microsoft administrator consent where required, sandbox/live validation, attachments, pagination, and webhook evidence |
...
1. Expand document ingestion and generation, including trusted legacy DOC conversion, OCR/layout fallbacks, tailored document variants, and submitted-document retention.
