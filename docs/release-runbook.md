...
The source milestone is `Durable Media Recovery v0.51.0-alpha.1`; the commands above are required checks, not a claim that its full suite, CI, installer, signing, or release-lab validation has already passed. Record final results against the exact reviewed commit and artifact hashes before publication.
