...
Do not reuse v0.60 counts, hashes or packaged-smoke results as v0.61 acceptance. Record the fresh `pnpm test` and `pnpm build` summaries, the backend collection total from `python -m pytest backend/tests --collect-only -q`, and the default desktop total separately from the opt-in delivered-supervisor run. The focused restore review may be repeated with:

```powershell
python -m pytest backend/tests/test_restore_gate_v2.py backend/tests/test_restore_rollback_v2.py backend/tests/test_mail_attempt_restore.py backend/tests/test_browser_worker_packaging.py backend/tests/test_version_sync.py
pnpm --filter @job-apply-pro/desktop test src/main/restore-recovery-controller.test.ts src/main/index-lifecycle.test.ts
```
