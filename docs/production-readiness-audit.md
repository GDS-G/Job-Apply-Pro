...
The unsigned local installer `Job-Apply-Pro-0.14.0-alpha.1-x64.exe` is 163,171,144 bytes with SHA-256 `64364A02ACBADAB9BD0E3407BA2A669DFC564D7F0DA81BC7E12454BCF15D6FD0`. Its bundled backend executable is 16,642,157 bytes with SHA-256 `7F6A68BD64829424645B65BED300B04AA419307D6BBE21B36C0826EB61929EE5`. Authenticode reports `NotSigned` for both, as expected for a local candidate built without release credentials. Generated update metadata identifies `0.14.0-alpha.1` and the matching installer size; it is validation evidence only and must not be published as a production update. Remote integration evidence is recorded only after the exact release commit completes required GitHub checks.

## Current release classification

`Supervised Portal Execution v0.14.0-alpha.1` is the current Windows alpha source candidate. It preserves the Portal Readiness, Provider Connectivity, and production-hardening baselines while adding a generic live-browser execution boundary for specifically allowlisted portals. It is not feature-complete or a stable production release: portions of document generation/import, portal-specific mappings, cross-platform packaging, provider attachment/pagination depth, and other traceability items remain source work. Signing, provider registration/authorization, legal/terms approval, authorized live evidence, and physical release-lab evidence remain external gates. Credentials pasted into chat are not an automation or secret-storage route; live validation requires owner-controlled browser sign-in, with MFA and one-time codes completed by the owner. The signed release workflow must not be dispatched and no portal catalog entry may set `production_enabled=true` until the relevant source and external controls exist.
