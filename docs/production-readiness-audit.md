...
`Provider Connectivity v0.13.0-alpha.1` is the current Windows alpha source candidate. It preserves the Portal Readiness and production-hardening baselines while adding encrypted OAuth/PKCE lifecycle management and official Gmail, Outlook, Google Calendar, and Outlook Calendar adapters. It is not feature-complete or a stable production release: named live-portal execution, portions of document generation/import, cross-platform packaging, provider attachment/pagination depth, and other traceability items remain source work, while signing, provider client registration/authorization, provider verification or administrator consent, authorized live evidence, and physical release-lab evidence remain external gates. Credentials pasted into chat are not a suitable automation or secret-storage route; live validation requires owner-controlled browser sign-in, with MFA and one-time codes completed by the owner. The signed release workflow must not be dispatched and no portal catalog entry may set `production_enabled=true` until the relevant source and external controls exist.
...
Until evidence groups 1-5 exist, the application remains alpha. Real portal submission stays disabled; mail/calendar writes require reviewed OAuth scopes, explicit mutation confirmation, and authorized validation and are not claimed as production-ready.
