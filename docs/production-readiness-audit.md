...
`Document Ingestion Resilience v0.16.0-alpha.1` is the current Windows alpha source candidate under validation. It preserves the Portal Readiness, Provider Connectivity, supervised execution, production-hardening, and document-generation baselines while adding bounded legacy DOC conversion and scanned-PDF OCR. It is not feature-complete or a stable production release: richer multi-column layout handling, portal-specific mappings, cross-platform packaging, provider attachment/pagination depth, and other traceability items remain source work. Signing, provider registration/authorization, legal/terms approval, authorized live evidence, owner-installed helper validation, and physical release-lab evidence remain external gates. Credentials pasted into chat are not an automation or secret-storage route; live validation requires owner-controlled browser sign-in, with MFA and one-time codes completed by the owner. The signed release workflow must not be dispatched and no portal catalog entry may set `production_enabled=true` until the relevant source and external controls exist.
