...
This guide applies to Reviewed LinkedIn Job Identity `v0.82.0-alpha.1`. This is an alpha milestone, not a production-ready signed release. On an already captured LinkedIn `JOB_DETAIL` page, choose **Review job identity** to request a fresh read-only proof. The card appears only when the backend still sees the same takeover session, exact URL, page fingerprint, job-detail classification, canonical `/jobs/view/<positive id>` path, and one visible bounded H1. It displays the normalized title, job ID, canonical URL, and shortened review fingerprint. It does not sign in, click, type, import or save the job, read a password, solve MFA/CAPTCHA, or submit an application. If the page changed, has tracking parameters or a fragment, uses another path, or contains zero or multiple H1 headings, capture a stable canonical page before trying again. Production portals remain disabled; controlled fixtures are not live-provider compatibility or legal/terms approval.
