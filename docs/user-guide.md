...
This guide applies to Reviewed Browser Link Navigation `v0.80.0-alpha.1`. This is an alpha milestone, not a production-ready signed release. The active build adds backend-derived, native-confirmed ordinary same-portal link navigation without clicking provider DOM elements. It strips and excludes query, fragment and embedded-credential values, excludes downloads and cross-portal targets, rechecks the exact page and target after resume, and uses exact URL verification. Changed or redirected results fail closed; an `UNCERTAIN` result is never permission to retry blindly. The inherited Greenhouse field/widget/upload/navigation/submission, recovery, profile, restore and portal safety boundaries remain unchanged. Named portal capability is disabled by default and is not a live compatibility claim. This build does not bulk fill, log in, solve MFA/CAPTCHA or assessments. Backend allowed actions and exact review fingerprints remain authoritative. Live providers, signed installers, physical Windows behavior and protected integrations retain their separate acceptance gates.
