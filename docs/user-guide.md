...
1. For production use, wait for a published signed installer. `Job-Apply-Pro-0.51.0-alpha.1-x64.exe` is the planned filename; this milestone does not yet have validated packaging evidence. Earlier unsigned development candidates are not signed releases. Test unsigned candidates only on an isolated development workstation.
