...
4. Confirm only if the fingerprint is still the reviewed value. The app stops its backend and recovery worker, checks unresolved media cleanup and preservation of mail-send history, applies the staged files offline only when both checks permit replacement, retains the prior database as `job-apply-pro.db.pre-restore`, migrates if required, and restarts.
