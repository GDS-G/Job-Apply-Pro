...
This guide applies to Reviewed Greenhouse Contenteditable Single-Select `v0.81.0-alpha.1`. This is an alpha milestone, not a production-ready signed release. The active build recognizes one additional Greenhouse widget representation: an expanded contenteditable `div` or `span` combobox that owns exactly one visible non-multiselect listbox and satisfies every inherited state, topology, locator, option-label and non-legal guard. After the existing field review and native approval, the worker rechecks the live widget and may click one exact visible option, then verifies the normalized displayed text. A changed, noneditable, collapsed, ambiguous, repeated, legal, signature or multiselect widget stays manual or fails closed; an `UNCERTAIN` result is never permission to retry blindly. The build does not provide generic content editing, open widgets, bulk fill, log in, solve MFA/CAPTCHA or assessments. Named portal capability remains disabled by default and controlled fixtures are not live-provider compatibility evidence. Backend allowed actions and exact review fingerprints remain authoritative. Live providers, signed installers, physical Windows behavior and protected integrations retain separate acceptance gates.
...
For an eligible uncertain navigation, Operations instead shows **Verify navigation outcome**. For a reviewed Greenhouse stage click, leave the visible browser at the result page in user takeover. The backend must independently observe the same portal origin and a recognized, strictly later non-confirmation stage with a different page fingerprint. Inspect the cancel-default native warning and choose **Record navigation outcome** only if the displayed transition is expected. The backend observes and proves the page again before recording reconciliation. It does not click, navigate, upload, submit, or retry; confirmation pages and same/earlier/unknown/foreign-origin pages remain unresolved. The original action and attempt stay `UNCERTAIN`, and resuming automation remains a separate deliberate action requiring a fresh capture.

For an eligible exact direct-URL navigation, leave the visible browser at the intended target page in user takeover and choose **Verify navigation outcome**. The backend preview identifies `EXACT_URL` scope only when the original action used one allowlisted HTTP(S) target and exact `URL_EQUALS` verification, the source and target URLs differ, and pre-dispatch encrypted intent exists. It independently requires the current URL and origin to equal that target exactly, a changed page fingerprint, and worker evidence that its prior action was `NAVIGATE`; approval repeats every check. Redirected, substring-only, already-current, manually browsed, stale, foreign-origin, or arbitrary link-click outcomes remain unresolved. The native warning never displays the full encrypted URL. Recording evidence does not navigate, click, retry, resume automation, or claim any unrelated provider result.
...
6. When **Reviewed direct links** appears, read the label and safe origin/path, choose **Review & navigate link**, and inspect the Cancel-default native warning. Only a backend-derived ordinary same-portal anchor is eligible. A query, fragment, embedded credential, download, JavaScript handler, changed page, redirect or cross-portal link remains manual. Approval navigates once to the exact target; it does not click the page element.
7. To test automated final submission, separately set `JAP_SUPERVISED_PORTAL_SUBMISSION_ENABLED=true`, restart, return to the same persistent profile, and capture the final review page again. Select **Review & submit exact application** only when the displayed application is correct. The native dialog is the last approval boundary.
8. Treat only an identifier-backed provider confirmation as confirmed. If the result is uncertain, the run records `SUBMISSION_UNCERTAIN`; verify directly in the provider account instead of retrying blindly.
9. Select **Stop & preserve trace** when complete. Remove the portal from the allowlist and disable both supervised gates after the approved window.
