...
Open **Reviewed email & verified attachments**, refresh mail drafts, select the source correspondence and owning workflow, and create a draft for review. Attachment drafts require a workflow so the app can verify profile ownership; recruiter-first messages without one currently support text-only drafts. Select up to four immutable PDF/DOCX versions totaling at most 2 MiB. Supported roles are resume, cover letter, certification, education and portfolio; `OTHER` is not supported. The app does not strip metadata or scan files for malware, so inspect the original files before selection.
...
Database restore also preserves mail-send history. It rejects a backup that would remove or change any recorded send attempt or draft claim, including failed, uncertain and historical confirmed attempts. Use a verified backup containing the same history; do not remove audits to force an older snapshot through. This prevents an old reviewed draft from becoming sendable again after restore.
...
4. Confirm only if the fingerprint is still the reviewed value. The app stops its backend and recovery worker, acquires exclusive workspace ownership, verifies closed storage, archive/staged bytes, unresolved media cleanup and preservation of mail-send history. It records encrypted intent and a persistent admission guard before replacing selected files. Each database replacement retains a fresh encrypted preimage under `restore-control/operations/<operation-id>/`.
5. Confirm the dashboard, candidate records, and backup status are expected before continuing work.

Successful completion verifies a receipt against the final files before removing the guard, migrating if required and restarting. If recovery is interrupted, startup and updates remain blocked even after a desktop restart. Preserve the workspace, encrypted backups, original master key and every `restore-control` record. Do not repeatedly restore, delete the guard or generate a replacement key. A missing key for an existing workspace is not automatically replaced.

The support commands `restore-status` and `restore-finalize --operation-id <uuid>` respectively inspect admission without opening SQLite and verify an already completed replacement using its authenticated receipt. Finalization cannot resume incomplete writes or roll back files. Recovery requires a supported app-owned local layout; external/redirected or cloud-synchronized layouts are not validated. Follow the release runbook and [ADR-0066](adr/0066-durable-restore-admission.md) before attempting recovery.
