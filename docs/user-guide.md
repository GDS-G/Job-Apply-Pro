...
This guide applies to Reviewed Greenhouse Single-Select Widgets `v0.70.0-alpha.1`. This is an alpha milestone, not a production-ready signed release: named portal capability is disabled by default and is not a live compatibility claim. Select one durable workflow in the queue; the guide and all application selectors follow that workflow's candidate, application, portal, challenge, and communication records. For an imported public Greenhouse job, complete requirements, qualification/eligibility, and immutable resume review until status is **ready**. Choose **Review Greenhouse launch**, verify the exact URL, origin, document version, and fingerprint, then choose **Open reviewed Greenhouse application** and approve the cancel-default native dialog. The backend revalidates everything and derives the URL. When a recognized Greenhouse form page is captured, the supervised-session card shows the current stage, required controls satisfied, field/upload/manual blockers, the unique next control when present, and each expected postcondition. Native-field entry, one narrowly reviewed single-select widget, required resume upload, ready one-stage navigation and manual handoff retain separate exact reviews. A qualifying widget is already expanded, visible, enabled and searchable, controls one visible non-multiselect listbox and exposes unique visible labels. Choose **Review field binding**, select a reviewed yes/no or multiple-choice answer, preview and approve the exact binding, then use **Review & populate exact field**. If it collapsed or changed, capture and review again; the app will not reopen it automatically. All other custom, multiselect, legal, signature, repeated, hidden or ambiguous controls remain manual. At the reviewed final page, the submit control is shown only when the current provider contract contains exactly one safe `FINAL_SUBMISSION_GATE`. Choose **Review & submit exact page**, inspect the cancel-default native confirmation, and approve only if the exact application is ready. Both global supervised-execution flags and the Greenhouse allowlist must be enabled. The backend freshly revalidates the allowed origin, exact page review, exact form review and semantic locator, then durably owns one click. Only a recognized same-origin provider confirmation page containing an identifier is success. Any other result is **submission uncertain**, requires visible-browser takeover and cannot retry automatically. This build still does not bulk fill, log in, solve MFA/CAPTCHA or assessments. A **Suggested** destination is navigation only—it does not grant a workflow action or establish readiness. Backend allowed actions and exact review fingerprints remain authoritative. Browser and AI provider actions retain durable pre-dispatch ownership; an `UNCERTAIN` result requires review or user takeover and is never permission to retry blindly. Native Windows notifications are implemented but still require physical release-lab validation. Live mail/calendar access remains unavailable until the owner registers an OAuth desktop client, imports reviewed registration metadata, and completes provider authorization. Calendar creation is CREATE-only, explicitly reviewed, no-invitation, one-attempt behavior; it is not unattended scheduling or proof of provider delivery. Live Gemini access likewise requires an owner-created API key, reviewed provider terms/privacy/retention settings, explicit external-AI consent, explicit media-upload consent for images, and a local uncommitted gateway configuration.
...
1. For production use, wait for a published signed installer. A future `Job-Apply-Pro-0.70.0-alpha.1-x64.exe` development-candidate filename would not by itself prove signing or successful acceptance. Consult the readiness audit for the exact version's build, hashes and limitations. Test unsigned candidates only on an isolated development workstation.
...
**Keep workspace blocked** is the default and safest choice when you are unsure. For an interrupted operation, **Roll back exact restore** requires the fingerprint returned by the authenticated inspection and returns only that operation's targets to their sealed pre-operation bytes. For an authenticated terminal `APPLIED` or `ROLLED_BACK` receipt, **Finalize exact restore state** only verifies the terminal targets and clears the guard. The app reinspects the terminal state and requires the guard to be clear, then closes so you can reopen normally. Rollback never merges later work, resumes the forward apply or accepts unknown target bytes. Legacy v1 evidence remains completion/manual-recovery only.
